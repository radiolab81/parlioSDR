...
By utilizing a dedicated FPGA (such as Lattice iCE40, Gowin Tang Nano, or Altera Cyclone boards) between the ESP32 and the DAC/ADC, the architecture transforms:

    I/Q via PARLIO: The ESP32 streams lightweight, baseband I/Q data over the PARLIO bus instead of heavy RF samples.

    Hardware Mixing: The generalized Verilog DSP core inside the FPGA takes these I/Q samples, applies a Hardware-NCO, and performs Digital Up/Down Conversion (DUC/DDC).

    RF Output: The FPGA directly drives the external high-speed DACs and ADCs.
